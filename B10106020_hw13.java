...
public class B10106020_hw13 {

    public static boolean contains(final Integer[] arr, final int key) {
        return Arrays.asList(arr).contains(key);
    }
    
    public static boolean runThis(String inputStr , boolean isOneNum) {
    	Pattern p = Pattern.compile("[0-9]{4}");//檢查 字串是否符合 0-9 的字元，且需要四個
    	if(isOneNum) {
    		p = Pattern.compile("[0-9]{1}");//檢查 字串是否符合 0-9 的字元
    	}else {
    		p = Pattern.compile("[0-9]{4}");//檢查 字串是否符合 0-9 的字元，且需要四個
    	}
    	Matcher m = p.matcher(inputStr);   
    	boolean b = m.matches(); //判斷是否符合
    	return b;
	...
	public static void main(String[] args) {
		...
		while(repeat) {
			...
			//int userInput = user.nextInt(); //如果不是輸入整數呢?
			String userInputStr = user.next();
			if(!runThis(userInputStr, true)) {
				continue;
			}
			...
			if(userInput==1) {
				...
				for(int m=1;m<11;m++) {
					...
					while(isIllegal){
						...
						theNum = user.next(); //處理使用者輸入
						if(!runThis(theNum, false)) {
							continue;
						}
				...
				System.out.println("請只輸入1, 2, or 3三種模式\n");
			}
		}
	}
}
